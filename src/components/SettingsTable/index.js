...
const getCablesAPI = async (p) => {
	...
	const response = await fetch(`http://134.79.206.193/smcaptar/getCables?offset=${p*20}`);
  ...
  return (
    <div style={{marginTop:20,marginLeft:20}}>
		       ...
		       >
		 		...
		 		<button className="buttonModalCables" onClick={() => console.log("Update Cable")}>Update Cable</button> 
		 		</div>
		 		</section>
		 		...
		 		<div style={{margin:50}}>
		 			...
		 			onChange={(e) =>
		 				handleSubmitUpdate(e,modalCable,key)
		 			}
		 		/>
		 		</div>
		 		</div>
