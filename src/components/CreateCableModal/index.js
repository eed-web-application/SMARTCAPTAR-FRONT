...
import {CreateCableOrder} from "../../testData/order"
...
function CreateCableModal(props) {
	...
	const CreateCable = async () =>{
		...
		await axios.post(`http://134.79.206.193/smcaptar/CreateCable?table=${"SMARTCAPTAR_UPLOAD"}`, {cable:newCable,user:props.user.USERNAME}).then(
			(response) => {
				setCableNum("")
				setCableType("")
				setType("")
	...
	const updateConnTypes = async (e) => {
			const response = await fetch(`http://134.79.206.193/smcaptar/getConnTypes?cableType=${e.target.value}`);
  ...
  >
	...
	<div style={{marginBottom:10,display:"flex",justifyContent:"space-between"}}>
                ...
                <div> 
                <button className="buttonModalCables" type ="button" onClick={(e) => {CreateCable()}}>Create Cable</button> 
					</div>
...
<div className={'top'}>
	...
	<select className={"input"} onChange={(e) =>{
		...
		{props.projects.map((key,index) => {
			return(
				<option value={JSON.stringify(key)}>{key.PROJECT_NAME}: {key.AREA}</option>
			)
		
})}
	</select> : key == "ORIGIN_CONNTYPE" || key == "DEST_CONNTYPE"  ?
	   <select className={"input"} onChange={(e) =>{
